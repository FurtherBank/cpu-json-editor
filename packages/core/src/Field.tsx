...
const menuActionSpace = (props: FieldProps, fieldInfo: IField) => {
  ...
  if (fatherInfo && fatherInfo.type === 'array') {
    ...
    if (index + 1 < fatherInfo.length! && index + 1 !== length)
...
const FieldBase = (props: FieldProps) => {
  ...
  const mergedEntrySchema = useMemo(
    () => ctx.getMergedSchema(schemaEntry),
    [ctx, schemaEntry],
  );

  const { valueEntry, ofOption } = useMemo(
    () => getValueEntry(data, schemaEntry, ctx),
    [data, schemaEntry, ctx],
  );

  const mergedValueSchema = useMemo(
    () => ctx.getMergedSchema(valueEntry),
    [ctx, valueEntry],
  );

  const errors = useSelector<
    StateWithHistory<CpuEditorState>,
  ...
  const { view: { type: schemaEntryViewType = null } = {} } =
    mergedEntrySchema || {};
  const {
    const: constValue,
    enum: enumValue,
    view: { type: valueEntryViewType = null } = {},
  } = mergedValueSchema || {};
  const valueType =
  ...
  // 菜单动作空间以及动作执行函数
  const space = useMemo(
    () => menuActionSpace(props, fieldInfo),
    [props, fieldInfo],
  );
  const menuActionHandlers = useMenuActionHandlers(
    ctx,
    route,
    field,
    fatherInfo,
  ...
  const valueComponent = (
    <EditionComponent {...props} fieldInfo={fieldInfo} key={'edition'} />
  );
