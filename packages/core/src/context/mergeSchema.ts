...
import { isShort, schemaIsShort, virtualSchemaProps } from './virtual';
...
export type MergedSchemaWithoutVirtual = Omit<
  JSONSchema,
  keyof processedSchemaProps | 'additionalItems'
> &
 ...
 * @returns
 */
export const mergeSchemaMap = (
  ctx: CpuEditorContext,
  map: Map<string, JSONSchema | boolean>,
) => {
          ...
          switch (key) {
            case 'type':
              ...
              result[key] = result[key]
                ? _.intersection(result[key], typeValue)
                : typeValue;
            ...
            case 'patternProperties':
              ...
              for (const propKey in value) {
                if (
                  Object.prototype.hasOwnProperty.call(value, propKey) &&
                  !result[key]![propKey]
                ) {
