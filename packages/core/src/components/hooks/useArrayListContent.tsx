...
export const useArrayListContent = (
  ...
  const {
    prefixItems: { length: prefixLength = undefined, ref: prefixRef = '' } = {},
    items,
  } = mergedValueSchema || {};
    ...
    const prefixIsShort = prefixLength
      ? Math.min(
          ...range(prefixLength).map((i) => {
            const { [isShort]: shortable, title } =
              ctx.getMergedSchema(addRef(prefixRef, i.toString())) || {};
            return shortable ? (title ? 1 : 2) : 0;
          }),
        )
      : 0;

    const { [isShort]: itemsShortable = false, title = undefined } = items
      ? ctx.getMergedSchema(items) || {}
      : {};

    const listShortLevel = [
      prefixIsShort,
      itemsShortable ? (title ? 1 : 2) : 0,
    ];
