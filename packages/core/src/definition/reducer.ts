      ...
      (s: CpuEditorState, a: CpuEditorAction) => {
        ...
        switch (type) {
          ...
          case 'change': // change 是对非对象值的改变
            if (field === undefined) {
              s.data = cloneDeep(value)
            } else if (fieldData instanceof Array || fieldData instanceof Object) fieldData[field] = cloneDeep(value)
          ...
          case 'delete':
            ...
            if (fieldData instanceof Array) {
              // 注意数组删除，后面元素是要前移的
              const index = parseInt(field)
              console.log(index, fieldData[index])

              remove(fieldData, (value: any, i: number) => i === index)
