 ...
 * @returns
 */
const menuActionSpace = (props: FieldProps, ctx: CpuEditorContext, fieldInfo: IField) => {
  const { fatherInfo, field, short } = props
  const { mergedValueSchema, errors } = fieldInfo
...
const FieldBase = (props: FieldProps) => {
  ...
  // 菜单动作空间以及动作执行函数
  const space = useMemo(() => menuActionSpace(props, ctx, fieldInfo), [props, fieldInfo, ctx])
