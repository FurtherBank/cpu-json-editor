...
import { addRef, getValueByPattern, jsonDataType } from '../utils';
...
export const getDefaultValue = (
      ...
      nowData = produce(nowData, (draft: any) => {
        for (const key of Object.keys(draft)) {
          if (
            ...
            (!patternProperties || !getValueByPattern(patternProperties, key))
          )
            delete draft[key];
        }
      });
  ...
  if (allowedTypes && allowedTypes.length > 0) {
    ...
    switch (type) {
      case 'object':
        const result =
          jsonDataType(nowData) === 'object' ? _.clone(nowData) : {};
      ...
      case 'array':
        const arrayResult =
          jsonDataType(nowData) === 'array' && items ? _.clone(nowData) : [];
